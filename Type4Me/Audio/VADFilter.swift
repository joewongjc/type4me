...
/// State machine that decides whether to emit audio chunks based on VAD probabilities.
...
final class VADFilter {
    ...
    /// VAD probability above this → speech detected.
    var speechThreshold: Float = 0.5

    /// VAD probability below this → silence detected (hysteresis to avoid flapping).
    ...
    /// Process a VAD probability and return whether the audio chunk should be emitted.
    /// - Parameter probability: Speech probability from SileroVAD, range [0, 1].
    /// - Returns: `true` if the chunk should be sent to ASR.
    func shouldEmit(probability: Float) -> Bool {
        switch state {
        case .waitingForSpeech:
        ...
        case .speaking:
            if probability < silenceThreshold {
                state = .maybeSilent
                silenceStart = Date()
                return true  // keep emitting during transition
            }
            return true
        ...
        case .maybeSilent:
            ...
            return true  // keep emitting while in grace period
        }
    }
